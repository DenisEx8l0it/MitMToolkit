...
def get_target_ip() -> str:
    while True:
        ...
        if target_ip:
            ...
            print(f"{Error} Пожалуйста, введите IP адреса!")
...
def start_arp_attack(interface: str, gateway: str, target_ip: str) -> None:
    print(f"\n{Success} Запуск ARP Spoofing атаки...")
...
def main() -> None:
    ...
    print(f"{Info} Настройка Denial Of Service атаки...")
    
    interface, gateway = get_network_info()
    target_ip = get_target_ip()
    
    print(f"\n{Warning} ПОДТВЕРЖДЕНИЕ АТАКИ")
    print(f"[!] Режим: Denial Of Service")
    ...
    start_arp_attack(interface, gateway, target_ip)
