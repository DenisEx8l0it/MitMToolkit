# 💣 MitMToolkit - Автоматизаиця MitM атак

![MitMToolkit](https://img.shields.io/badge/Version-2.0-blue)
![Python](https://img.shields.io/badge/Python-3.8+-green)
![Linux](https://img.shields.io/badge/Linux-Fedora%20%7C%20Kali%20%7C%20Ubuntu-red)
![License](https://img.shields.io/badge/License-MIT-blue)
![Security](https://img.shields.io/badge/Security-Penetration%20Testing-orange)

**MitMToolkit - Инструмент для автоматизация ARP-Spoofing и DoS-Forwarding Disable атак... 🌐**

![Скриншот интерфейса main.py](https://github.com/DenisSh33l4r/MitMToolkit/blob/main/image/MITM.png)

---

## 📖 Описание

**MitMToolkit** - Инструмент для тестирования сетевых атак с поддержкой ARP-Spoofing и Denial of Service (Путем Отлючения IP Forwarding) 🌐

### Проект включает два основных модуля атак:
- **ARP Spoofing** - перехват сетевого трафика через подмену ARP-таблиц
- **ARP Poisoning DoS** - разрыв соединений между целью и шлюзом

### Идеально подходит для:
- Анализа уязвимостей сети
- Образовательных целей и исследований
- Проверки защищенности сетевой инфраструктуры

---

## ✨ Основные возможности

### 🔍 ARP Spoofing
- **Перехват трафика** между узлами сети
- **Манипуляция ARP-таблицами** для перенаправления пакетов
- **Поддержка различных сетевых конфигураций**

![Скриншот интерфейса ARP.py](https://github.com/DenisSh33l4r/MitMToolkit/blob/main/image/ARP.png)

##

### 📌 ARP Poisoning DoS (Denial of Service)
- **Отказ в обслуживании** через подмену ARP таблиц
- **IP forwarding отключен** - трафик теряется
- **Разрыв соединений** между целью и шлюзом

![Скриншот интерфейса DNS.py](https://github.com/DenisSh33l4r/MitMToolkit/blob/main/image/DoS.png)

##

### 💻 Профессиональный интерфейс
- **Цветной консольный интерфейс** с ASCII-графикой
- **Интуитивное меню** выбора типа атаки
- **Детальная информация** о процессе выполнения

##

### ⛔ Безопасность и контроль
- **Требует права root** для работы с сетевыми интерфейсами
- **Четкие предупреждения** о предназначении для образовательных целей
- **Контроль ошибок** и обработка исключений

---

## 🛠 Технологии

### Coding
- **Python 3.8+** - основной язык программирования
- **Colorama** - цветной вывод в консоль
- **Subprocess** - управление внешними процессами

### Сетевые возможности
- **arpspoof (из пакета dsniff)** - основной инструмент ARP атак
- **Raw socket manipulation** - работа с сетевыми пакетами
- **ARP protocol handling** - манипуляция ARP-запросами
- **iproute2** - управление сетевыми настройками

---

## ⚡ Быстрый старт

## Клонирование репозитория
```bash
git clone https://github.com/DenisEx8l0it/MitMToolkit.git
cd MitMToolkit
```

## Установка зависимостей
```bash
pip3 install -r requirements.txt
```

## Запуск программы
```bash
sudo python3 main.py
```
---

### ⚠️ Важное предупреждение
- **Только для легального использования!**

##

## 🏗 Структура проекта
```text
MitMToolkit/
├── main.py                # Основной запускаемый файл
├── ARP-Spoofing/
│   └── ARP.py             # Модуль ARP спуфинга
├── DoS-Attack/
│   └── DoS.py             # Модуль DoS атаки
├── README.md              # Документация
│
└── requirements.txt       # Библиотеки
```
---

### 🔧 Требования
- **Python 3.8+**
- **Права root/sudo**
- **Linux/Unix система**
- **Сетевой интерфейс с поддержкой promiscuous mode**

---

## 🔗 Ссылка на автора: https://github.com/DenisEx8l0it
